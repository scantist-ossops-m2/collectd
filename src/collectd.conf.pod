...
=item B<Priority> I<Num>

Set nice value (which affects scheduling priority) for the daemon process.
The priority is set similarly to the nice command: a negative number means
higher priority. Allowed range normally varies from -20 to 20.
Defaults to 0.
...
data source. When no types are configured, plugin checks for I<uptime> metric.

=item B<StartupTimeout> I<Value>

=item B<StartupThreshold> I<Value>

When Collectd restarts, cache is empty. This can result sending unnecessary
notifications for each running host/service. These options allow to avoid this.

During B<StartupTimeout> seconds after Collectd start, it will send OK
notifications only for services which uptime is less than B<StartupThreshold>
(i.e. which was (re-)started while Collectd was offline).

B<StartupTimeout> defaults to 60 seconds.
B<StartupThreshold> defaults to 3600 seconds.
...
=item B<SlaveThreadsState> I<true|false>

Enable collection of I/O and SQL threads running state.
Option requires B<SlaveStats> to be enabled.
Defaults to B<false>.
...
=item B<HostIsPluginInstance> B<true>|B<false>

Option allows to report B<Host> as I<plugin instance>.
Otherwise it will be reported as I<type instance>.
Defaults to B<false> (report metrics as I<type instance>).
...
=item B<ReportNVME> B<true>|B<false>

Enables NVMe devices information report.
Defaults to B<false>.
