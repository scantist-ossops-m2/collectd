...
This not applied to metrics which are reported as types other than
"protocol_counter". Look into "/proc/net/snmp" and "/proc/net/netstat" files
to find their appropriate I<Selector>.
...
=item B<ReportEmptyMetrics> B<true>|B<false>

When set to B<true>, allows reporting of metrics which source value is zero.
Defaults to B<false>.
